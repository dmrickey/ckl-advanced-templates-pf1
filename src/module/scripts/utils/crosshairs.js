 ...
 */
const containsCenter = (placeable, crosshairsData) => {
...
* @param {Function} [containedFilter=containsCenter]. Optional function for determining if a placeable
*   is contained by the crosshairs. Default function tests for centerpoint containment. {@link containsCenter}
*
* @return {Object<String,PlaceableObject>} List of collected placeables keyed by embeddedName
*/
const collectPlaceables = (
    crosshairsData,
    types = "Token",
    containedFilter = containsCenter
) => {
...
const showCrosshairs = async (config = {}, callbacks = {}) => {
    /* add in defaults */
    mergeObject(config, crosshairsConfig(), { overwrite: false });
